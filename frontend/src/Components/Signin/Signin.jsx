...
const Signin = () => {
  ...
  const bgImages = [signinImg1, signinImg2, signinImg3];
  ...
  return (
    <div className="h-screen w-full bg-slate-50 relative flex items-center justify-center p-4 font-sans text-slate-800 overflow-hidden">
      <Link to="/" className="absolute top-6 left-6 z-20 bg-white/70 backdrop-blur-md hover:bg-white text-slate-700 hover:text-indigo-600 p-2.5 rounded-full shadow-lg transition-all transform hover:scale-110">
      ...
      <div className="absolute top-0 left-0 w-full h-[50dvh] z-0 bg-slate-200">
        {bgImages.map((img, index) => (
          <img key={index} src={img} alt="Background" className={`absolute inset-0 w-full h-full object-cover object-top transition-opacity duration-1000 ease-in-out ${currentImageIndex === index ? 'opacity-100' : 'opacity-0'}`} />
        ))}
        <div className="absolute inset-0 bg-gradient-to-b from-black/5 via-transparent to-slate-50"></div>
      </div>

      <div className={`z-10 w-full max-w-md bg-white rounded-3xl p-7 sm:p-9 relative overflow-hidden transition-all duration-1000 ease-out transform shadow-[0_25px_60px_-15px_rgba(0,0,0,0.12),0_15px_30px_-10px_rgba(0,0,0,0.08)] ${isMounted ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95'}`}>
        {isSuccess && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/95 backdrop-blur-md transition-opacity duration-500">
            <div className="w-20 h-20 bg-teal-100 rounded-full flex items-center justify-center mb-4 shadow-xl animate-bounce">
              <svg className="w-10 h-10 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all text-sm bg-slate-50 hover:bg-white hover:border-slate-300 ${fieldErrors.email ? 'border-red-400 focus:border-red-500 focus:ring-red-100' : ''}`}
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all text-sm bg-slate-50 hover:bg-white hover:border-slate-300 ${fieldErrors.password ? 'border-red-400 focus:border-red-500 focus:ring-red-100' : ''}`}
              placeholder="••••••••"
            />
        ...
        <div className="mt-6 text-center text-xs text-slate-600 font-medium">
          Don't have an account? <Link to="/signup" className="font-bold text-indigo-700 hover:text-teal-600 transition-colors">Sign up here</Link>
        </div>
      </div>
    </div>
  );
